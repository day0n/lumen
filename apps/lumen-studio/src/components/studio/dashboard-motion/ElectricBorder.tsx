...
/**
 * Adapted from React Bits (https://reactbits.dev/animations/electric-border) — ElectricBorder-TS-TW
 */
import {
  useCallback,
  useEffect,
  ...
  const getRoundedRectPoint = useCallback(
    (t: number, left: number, top: number, width: number, height: number, radius: number) => {
      ...
      return (
        ...
        segment(straightHeight, (p) => ({ x: left + width, y: top + radius + p * straightHeight })) ??
        segment(cornerArc, (p) =>
        ...
        getCornerPoint(left + radius, top + radius, radius, Math.PI, Math.PI / 2, (distance - accumulated) / cornerArc)
      );
    },
    [getCornerPoint],
  );
