...
/**
 * PipelineStepper —— 横向步骤条，每步圆圈 + 标签 + 连接线。
 *
 * 从 reactbits.dev Stepper (TS+TW) 抽出 StepIndicator + StepConnector 两个
 * 子组件改造而来 —— 原版 Stepper 是个 wizard 包整体（带步骤内容 + 上一步
 * 下一步按钮），不适合我们的场景（pipeline 的步骤内容由外部管理）。
 * 我们这里只用它的"圆圈 + 连接线 + 平滑过渡"。
 *
 * 改造：
 * - 支持 stage 的多状态：locked / ready / running / success / error / cancelled。
 *   每种状态对应不同圆圈颜色和图标，覆盖 reactbits 原来只有 inactive/active/complete
 *   三态的语义。
 * - 圆圈下方显示步骤名标签，超出宽度 truncate。
 * - active 步骤的圆圈和标签都高亮，给用户当前位置感。
 * - locked 步骤的圆圈和连接线灰化且 pointer-events:none。
 * - 圆圈点击触发 onStepClick，由外部决定要不要切换 activeStep（仍受 locked 限制）。
 */
