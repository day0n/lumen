'use client';

import { useI18n } from '@/i18n/provider';

export function BgmTrack({ bgmUrl, volume }: { bgmUrl: string | null; volume: number }) {
  const { t } = useI18n();

  if (!bgmUrl) {
    return (
      <div className="flex h-10 items-center rounded-[8px] border border-dashed border-white/[0.1] px-3 text-[11px] text-white/34">
        {t('canvas.composition.noBgm')}
      </div>
    );
  }

  return (
    <div className="flex h-10 items-center gap-2 rounded-[8px] bg-[#2a2418] px-3 ring-1 ring-[#f5c76a]/18">
      <span className="text-[11px] font-bold text-[#ffd88a]">BGM</span>
      <span className="min-w-0 flex-1 truncate text-[10px] text-white/48">{bgmUrl}</span>
      <span className="text-[10px] text-white/42">{Math.round(volume * 100)}%</span>
    </div>
  );
}
