...
function withCanvasOutputMetrics(nodes: LumenNode[], edges: LumenEdge[]) {
  ...
  return {
    ...
    // downstream node memoization useless. Now untouched
  ...
  useEffect(() => {
    ...
    async function createProject() {
      try {
        ...
        // 用 history.replaceState 取代框架路由替换：
        // 后者会触发完整路由切换，导致加载页再现一次并让工作区重挂载，
...
// 状态选择器直接返回布尔值，只在跨过缩放阈值时重渲本组件和切换节点细节。
...
function LumenFlowNodeImpl({ data, id, selected }: NodeProps<LumenNode>) {
  ...
  const handlePromptKeyDown = useCallback(
    (event: ReactKeyboardEvent<HTMLTextAreaElement>) => {
      // Prevent the canvas keyboard handler from intercepting text input.
      event.stopPropagation();
...
// Memoize custom node components. Without this wrapper, every state change
// anywhere in the canvas (a ws node:progress
